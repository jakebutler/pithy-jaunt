...
export const Route = createFileRoute('/api/debug/task/$taskId')({
  server: {
    handlers: {
      GET: async ({ params }) => {
        try {
...
function getRecommendations(
  ...
  if (task.status === 'running' && runningMinutes > 10) {
    recommendations.push(
      'Task has been running for over 10 minutes, which is unusual for simple tasks'
    )

    if (!workspace) {
      recommendations.push(
        'Workspace was not found in database despite having workspace ID assigned'
      )
      recommendations.push('This suggests workspace creation failed silently')
    } else if (workspace.status === 'creating') {
      recommendations.push(
        "Workspace is still in 'creating' state - may be stuck provisioning"
      )
    }
    ...
    recommendations.push(
      'Consider canceling and checking: DAYTONA_API_URL, DAYTONA_API_KEY, WEBHOOK_URL'
    )
  }
