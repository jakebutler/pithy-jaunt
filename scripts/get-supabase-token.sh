...
echo ""
echo "Method 3: From Browser Console (if Supabase is available)"
echo "------------------------------------------"
echo "1. Open DevTools → Console tab"
echo "2. Try this JavaScript (may not work if client isn't exposed):"
echo ""
echo "   // Option A: If window.supabase exists"
echo "   const { data: { session } } = await window.supabase.auth.getSession()"
echo "   console.log('Access Token:', session?.access_token)"
echo ""
echo "   // Option B: Check localStorage directly"
echo "   const storageKey = Object.keys(localStorage).find(k => k.includes('auth-token'))"
echo "   if (storageKey) {"
echo "     const session = JSON.parse(localStorage.getItem(storageKey))"
echo "     console.log('Access Token:', session?.access_token)"
echo "   }"
echo ""
